...
sumItemVar <- sum(aggregate(itemParaRep$PQwtd, by=list(Category=itemParaRep$b), FUN=sum)$x)
sumItemVar


#cronbachAlpha

2.202261 + 1.883482 + 1.195212

cronbachAlpha <- 31 / (31 - 1) * (19.28792 - 5.280955)/19.28792
...
# Coefficient alpha	0.730	0.719
# Feldt's coefficient	0.731	0.720
# Stratified Alpha	0.735	0.722
...
itemPara <- itemPara_BF[,c("b", "ag", "ai")]
...
# replicate item parameter and theta
itemParaRep <-itemPara[rep(seq_len(numOfItem), each = numOfQuad^4),]
itemParaRep[,c(4:8)]<- rep(nodesM[,c(1:5)], each = 1, length.out = numOfQuad^4*numOfItem)
# itemParaRep$thetai <- rep(quadPoints[,c(2)], each = 1, length.out = numOfQuad^4*numOfItem)
